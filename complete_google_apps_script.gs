...
function importGFSData(spreadsheet) {
  ...
  const possibleSheetNames = ['GFS Order July 4 2025', 'Sheet1', 'GFS_Order_July4_2025'];
...
function createCustomMenu() {
  ...
  ui.createMenu('🏔️ Camp Inventory')
    ...
    .addSeparator()
    .addItem('🔧 Import GFS Data', 'directImportGFS')
...
function directImportGFS() {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const sourceSheet = spreadsheet.getSheetByName('GFS Order July 4 2025');
  const targetSheet = spreadsheet.getSheetByName('Inventory_Master');
  
  if (!sourceSheet) {
    SpreadsheetApp.getUi().alert('Cannot find "GFS Order July 4 2025" sheet!');
    return;
  }
  ...
  // Process and prepare data for Inventory_Master
  const processedData = [];
  ...
  // Write data to Inventory_Master
  if (processedData.length > 0) {
    ...
    for (let i = 2; i <= processedData.length + 1; i++) {
      ...
      targetSheet.getRange(i, 14).setFormula('=IFERROR(1,1)'); // Simplified for now
    ...
    SpreadsheetApp.getUi().alert('Success!', `Imported ${processedData.length} products to Inventory_Master`, SpreadsheetApp.getUi().ButtonSet.OK);
  } else {
...
function debugImport() {
  ...
  if (!sourceSheet) {
    SpreadsheetApp.getUi().alert('Sheet "GFS Order July 4 2025" not found!');
    return;
  }
  ...
  message += `First row data: ${JSON.stringify(firstDataRow)}`;
  
  SpreadsheetApp.getUi().alert('Debug Info', message, SpreadsheetApp.getUi().ButtonSet.OK);
}

// ===== MANUAL IMPORT FUNCTION =====
function importGFSDataManual() {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const sourceSheetName = 'GFS Order July 4 2025'; // Your actual sheet name
  const sourceSheet = spreadsheet.getSheetByName(sourceSheetName);
  const targetSheet = spreadsheet.getSheetByName('Inventory_Master');
  
  if (!sourceSheet) {
    SpreadsheetApp.getUi().alert('Source sheet not found! Change the sheet name in the code.');
    return;
  }
  ...
  // Import to Inventory_Master
  importGFSData(spreadsheet);
  
  SpreadsheetApp.getUi().alert('Import complete! Check Inventory_Master sheet.');
}
