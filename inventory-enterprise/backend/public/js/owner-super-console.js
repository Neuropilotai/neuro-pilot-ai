...
async function loadDashboard() {
  ...
  try {
    ...
    loadCognitiveIntelligence();
    loadActivityFeed();
    // v22: Load AI Scheduler Status
    loadAISchedulerStatus();
    // v22.1: Load Governance Trends
    loadGovernanceTrends();
...
// ============================================================================
// v22: AI SCHEDULER STATUS
// ============================================================================

/**
 * Load AI Scheduler Status Card
 * Fetches lightweight status from /api/owner/ops/ai-status
 */
async function loadAISchedulerStatus() {
  const dotEl = document.getElementById('schedulerDot');
  const statusEl = document.getElementById('schedulerStatusText');
  const forecastEl = document.getElementById('lastForecastTime');
  const learningEl = document.getElementById('lastLearningTime');
  const governanceEl = document.getElementById('lastGovernanceTime');

  // Skip if elements don't exist (e.g., card not rendered)
  if (!dotEl || !statusEl) {
    console.debug('AI Scheduler card not present, skipping');
    return;
  }

  try {
    const data = await fetchAPI('/owner/ops/ai-status');
    ...
    const color = data.health?.color || 'gray';
    const colorMap = {
      green: '#22c55e',
      yellow: '#f59e0b',
      red: '#ef4444',
    ...
    dotEl.style.color = colorMap[color] || colorMap.gray;
    statusEl.textContent = data.health?.status || 'Unknown';

    // Update timing
    if (forecastEl) {
      forecastEl.textContent = data.timing?.forecastAgeHuman || 'Never';
    }
    if (learningEl) {
      learningEl.textContent = data.timing?.learningAgeHuman || 'Never';
    }
    // v22: Update governance timing
    if (governanceEl) {
      governanceEl.textContent = data.timing?.governanceAgeHuman || 'Never';
    }

    // Update button states based on active jobs
    const activeJobs = data.scheduler?.activeJobs || [];
    updateJobButton('btnForecast', 'ai_forecast', activeJobs);
    updateJobButton('btnLearning', 'ai_learning', activeJobs);
    updateJobButton('btnGovernanceScore', 'governance_score', activeJobs);
    updateJobButton('btnSelfHeal', 'self_heal', activeJobs);

    // v22: Load job metrics after scheduler status
    loadJobMetrics();

  } catch (error) {
    console.error('Failed to load AI scheduler status:', error);
...
/**
 * Update job button state based on active jobs
 */
function updateJobButton(btnId, jobName, activeJobs) {
...
/**
 * Trigger an AI job manually
 * @param {string} jobName - ai_forecast, ai_learning, self_heal, governance_score
 */
async function triggerAIJob(jobName) {
  const logEl = document.getElementById('aiSchedulerLog');
  const btn = document.getElementById('btn' + jobName.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(''));
  ...
  if (logEl) {
    ...
    logEl.innerHTML = `<div class="log-entry log-info">[${timestamp}] Triggering ${jobName}...</div>` + logEl.innerHTML;
  }

  try {
    const result = await fetchAPI(`/owner/ops/trigger/${jobName}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }
    });
    ...
    if (result.success) {
      ...
      if (logEl) {
        const timestamp = new Date().toLocaleTimeString();
        logEl.innerHTML = `<div class="log-entry log-success">[${timestamp}] ✅ ${jobName} completed${duration}</div>` + logEl.innerHTML;
      }
    ...
    setTimeout(loadAISchedulerStatus, 1000);

  } catch (error) {
    console.error(`Failed to trigger ${jobName}:`, error);
    if (logEl) {
      const timestamp = new Date().toLocaleTimeString();
      logEl.innerHTML = `<div class="log-entry log-error">[${timestamp}] ❌ ${jobName} failed: ${error.message}</div>` + logEl.innerHTML;
    }
    showToast(`Failed to trigger ${jobName}: ${error.message}`, 'error');
  } finally {
    // Re-enable button
    if (btn) {
      btn.disabled = false;
...
/**
 * v22: Load job metrics from API
 */
async function loadJobMetrics() {
  const gridEl = document.getElementById('jobMetricsGrid');
  const breakerPanel = document.getElementById('circuitBreakerPanel');
  const breakerList = document.getElementById('circuitBreakerList');

  if (!gridEl) return;

  try {
    const data = await fetchAPI('/owner/ops/job-metrics');

    if (!data.success) {
      throw new Error(data.error || 'Failed to fetch job metrics');
    }

    // Render job metric cards
    if (data.jobs && data.jobs.length > 0) {
      gridEl.innerHTML = data.jobs.map(job => `
        <div class="job-metric-card ${job.health}">
          <div class="job-metric-name">${formatJobName(job.name)}</div>
          <div class="job-metric-stats">
            <div class="job-metric-stat">
              <span class="job-metric-stat-label">Runs:</span>
              <span class="job-metric-stat-value">${job.runCount}</span>
            </div>
            <div class="job-metric-stat">
              <span class="job-metric-stat-label">Errors:</span>
              <span class="job-metric-stat-value ${job.errorCount > 0 ? 'error' : ''}">${job.errorCount}</span>
            </div>
            <div class="job-metric-stat">
              <span class="job-metric-stat-label">Success:</span>
              <span class="job-metric-stat-value ${job.successRate === '100%' ? 'success' : ''}">${job.successRate}</span>
            </div>
            <div class="job-metric-stat">
              <span class="job-metric-stat-label">Avg:</span>
              <span class="job-metric-stat-value">${job.avgDuration}</span>
            </div>
            <div class="job-metric-stat">
              <span class="job-metric-stat-label">Timeouts:</span>
              <span class="job-metric-stat-value ${job.timeoutCount > 0 ? 'error' : ''}">${job.timeoutCount}</span>
            </div>
            <div class="job-metric-stat">
              <span class="job-metric-stat-label">Retries:</span>
              <span class="job-metric-stat-value ${job.retryCount > 0 ? 'warning' : ''}">${job.retryCount}</span>
            </div>
          </div>
          ${job.config ? `<div class="job-metric-config">Timeout: ${Math.round(job.config.timeoutMs/1000)}s | Max Retries: ${job.config.maxRetries}</div>` : ''}
        </div>
      `).join('');
    } else {
      gridEl.innerHTML = '<div class="job-metric-loading">No job runs yet</div>';
    }

    // Render circuit breakers if any are tripped
    const breakers = data.circuitBreakers || {};
    const trippedBreakers = Object.entries(breakers).filter(([, b]) => b.state !== 'closed');

    if (trippedBreakers.length > 0 && breakerPanel && breakerList) {
      breakerPanel.style.display = 'block';
      breakerList.innerHTML = trippedBreakers.map(([jobName, breaker]) => `
        <div class="circuit-breaker-item">
          <span class="circuit-breaker-job">${formatJobName(jobName)}</span>
          <div class="circuit-breaker-state">
            <span class="circuit-breaker-badge ${breaker.state}">${breaker.state.toUpperCase()}</span>
            <button type="button" class="btn btn-xs btn-outline" onclick="resetCircuitBreaker('${jobName}')">Reset</button>
          </div>
        </div>
      `).join('');
    } else if (breakerPanel) {
      breakerPanel.style.display = 'none';
    }

  } catch (error) {
    console.error('Failed to load job metrics:', error);
    gridEl.innerHTML = '<div class="job-metric-loading">Failed to load metrics</div>';
  }
}

/**
 * v22: Reset circuit breaker for a job
 */
async function resetCircuitBreaker(jobName) {
  try {
    const result = await fetchAPI(`/owner/ops/reset-circuit-breaker/${jobName}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }
    });

    if (result.success) {
      showToast(`Circuit breaker reset for ${formatJobName(jobName)}`, 'success');
      loadJobMetrics();
    } else {
    ...
    showToast(`Failed to reset circuit breaker: ${error.message}`, 'error');
  }
}
...
// Export AI scheduler functions to global scope
window.loadAISchedulerStatus = loadAISchedulerStatus;
window.triggerAIJob = triggerAIJob;
window.loadJobMetrics = loadJobMetrics;
window.resetCircuitBreaker = resetCircuitBreaker;

// ============================================================================
// v22.1: GOVERNANCE TRENDS
// ============================================================================

/**
 * Load governance trends and render sparklines
 */
async function loadGovernanceTrends() {
  ...
  try {
    const data = await fetchAPI(`/governance/trends?days=${days}`);

    if (!data.success) {
      throw new Error(data.error || 'Failed to fetch trends');
    }

    // Pillar mapping for element IDs
    const pillarMap = {
      composite: 'Composite',
      finance: 'Finance',
      health: 'Health',
    ...
    // Update each pillar's trend card
    for (const [pillar, displayName] of Object.entries(pillarMap)) {
      const trendData = data.trends[pillar] || [];
      const summary = data.summary[pillar] || {};

      // Update value
      const valueEl = document.getElementById(`trend${displayName}Value`);
      if (valueEl) {
        valueEl.textContent = summary.current !== null ? `${summary.current}%` : '--';
      }

      // Update trend indicator
      const indicatorEl = document.getElementById(`trend${displayName}Indicator`);
      if (indicatorEl) {
        indicatorEl.className = 'trend-indicator ' + (summary.trend || 'stable');
      }

      // Update range
      const rangeEl = document.getElementById(`trend${displayName}Range`);
      if (rangeEl && summary.min !== null) {
        rangeEl.textContent = `${summary.min}% - ${summary.max}% (avg: ${summary.avg}%)`;
      }

      // Render sparkline
      const sparklineEl = document.getElementById(`sparkline${displayName}`);
      if (sparklineEl && trendData.length > 0) {
        renderSparkline(sparklineEl, trendData.map(d => d.score));
      } else if (sparklineEl) {
        sparklineEl.innerHTML = '<text x="60" y="25" fill="#6b7280" text-anchor="middle" font-size="10">No data</text>';
      }
    }

  } catch (error) {
    console.error('Failed to load governance trends:', error);
  }
}
 ...
 * @param {number[]} data - Array of score values
 */
...
// Export governance trends functions
