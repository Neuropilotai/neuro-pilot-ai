/**
 * Prophet Forecaster
 * Facebook Prophet time-series forecasting implementation
 * Best for: seasonal patterns, holidays, strong trends
 */
...
class ProphetForecaster extends BaseForecaster {
  constructor(config = {}) {
    super(config);
    this.modelType = 'prophet';
    this.scriptPath = path.join(__dirname, '../python/train_prophet.py');
  }
  ...
  async train(itemCode, options = {}) {
    try {
      logger.info('Training Prophet model', { itemCode, options });
      ...
      if (trainingData.length < 10) {
        logger.warn('Insufficient training data for Prophet', {
          itemCode,
      ...
      const config = {
        ...
        seasonality_mode: options.seasonalityMode || 'multiplicative',
        changepoint_prior_scale: options.changepointPriorScale || 0.05,
        seasonality_prior_scale: options.seasonalityPriorScale || 10.0,
        yearly_seasonality: options.yearlySeasonality !== false,
        weekly_seasonality: options.weeklySeasonality !== false,
      ...
      if (!result.success) {
        logger.error('Prophet training failed', {
          itemCode,
          error: result.error
        });
        return result;
      ...
      logger.error('Prophet training error', {
        itemCode,
        error: error.message,
        stack: error.stack
      });
  ...
  /**
   * Generate predictions using existing Prophet model
   * @param {string} itemCode - Item code or 'global'
  ...
  async predict(itemCode, periods = 30) {
    try {
      logger.info('Generating Prophet predictions', { itemCode, periods });
      ...
      if (!model) {
        logger.warn('No trained Prophet model found', { itemCode });
        return {
          success: false,
      ...
      if (!result.success) {
        logger.error('Prophet prediction failed', {
          itemCode,
          error: result.error
        });
        return result;
      ...
      logger.info('Prophet predictions generated', {
        itemCode,
        forecast_count: result.forecast.length
      });
      ...
      logger.error('Prophet prediction error', {
        itemCode,
        error: error.message,
        stack: error.stack
      });
  ...
  async evaluate(itemCode, backtestDays = 30) {
    try {
      logger.info('Evaluating Prophet model', { itemCode, backtestDays });
      ...
      logger.info('Prophet model evaluation complete', {
        itemCode,
        mape: mape.toFixed(2),
      ...
      logger.error('Prophet evaluation error', {
        itemCode,
        error: error.message
      });
      return {
