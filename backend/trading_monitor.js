...
class TradingMonitor {
  ...
  async executeTrade(symbol, side, quantity, price) {
    ...
    if (side === "BUY") {
      ...
      // Update position
      const currentPosition = this.paperWallet.positions.get(symbol) || {
        quantity: 0,
        avgPrice: 0,
      };
      const newQuantity = currentPosition.quantity + quantity;
      const newAvgPrice =
        (currentPosition.quantity * currentPosition.avgPrice + cost) /
        newQuantity;
    ...
    console.log(
      `✅ ${side} ${quantity} ${symbol} @ $${price} | P&L: $${trade.pnl.toFixed(2)}`,
    );
    console.log(
      `💰 Balance: $${this.paperWallet.currentBalance.toFixed(2)} | Daily P&L: $${this.paperWallet.dailyPnL.toFixed(2)}`,
    );
  ...
  async getResults() {
    ...
    const todayTrades = this.paperWallet.trades.filter(
      (t) => new Date(t.time) >= todayStart,
    );
    ...
    return {
      ...
      positions: Array.from(this.paperWallet.positions.entries()).map(
        ([symbol, pos]) => ({
          symbol,
        ...
        for (const [sym, pos] of this.paperWallet.positions) {
          if (pos.quantity > 0) {
            await this.executeTrade(
              sym,
              "SELL",
              Math.min(quantity, pos.quantity),
              basePrice[sym] + priceVariation,
            );
            return;
          }
        }
...
if (require.main === module) {
  ...
  (async () => {
    ...
    await monitor.executeTrade("AAPL", "BUY", 100, 185.5);
    await new Promise((r) => setTimeout(r, 1000));

    await monitor.executeTrade("TSLA", "BUY", 50, 245.8);
    await new Promise((r) => setTimeout(r, 1000));

    await monitor.executeTrade("AAPL", "SELL", 50, 186.25);
    await new Promise((r) => setTimeout(r, 1000));

    await monitor.executeTrade("NVDA", "BUY", 25, 875.0);
