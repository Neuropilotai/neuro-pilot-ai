...
class AIJobMatchingEngine {
  ...
  async findJobMatches(resumeData, preferences = {}) {
    try {
      ...
      // Sort by match score
      matchedJobs.sort(
        (a, b) => b.matchScore.overallScore - a.matchScore.overallScore,
      );
  ...
  async skillsMatching(resumeData, job) {
    const candidateSkills = (resumeData.keywords || "")
      .toLowerCase()
      .split(",")
      .map((s) => s.trim());
    ...
    const score = Math.min(
      100,
      (matchCount / Math.max(jobRequirements.length, 1)) * 100,
    );
  ...
  async experienceMatching(resumeData, job) {
    ...
    if (
      resumeData.industry &&
      resumeData.industry.toLowerCase() === job.company.toLowerCase()
    ) {
      score += 20;
    }
  ...
  async salaryMatching(resumeData, job) {
    ...
    const baseExpectedSalary =
      80000 + this.parseExperience(resumeData.experience || "0") * 15000;
  ...
  generateRecommendationReason(matchScore) {
    ...
    return reasons.length > 0
      ? reasons.join(", ")
      : "Well-rounded match across multiple criteria";
  }
  ...
  generateActionItems(resumeData, job, matchScore) {
    ...
    if (breakdown.skills_matcher.score < 70) {
      actions.push(
        "Consider highlighting transferable skills more prominently",
      );
    }
  ...
  calculateAverageScore(matchedJobs) {
    ...
    const total = matchedJobs.reduce(
      (sum, match) => sum + match.matchScore.overallScore,
      0,
    );
  ...
  generateCandidateRecommendations(resumeData, matchedJobs) {
    ...
    if (matchedJobs.length === 0) {
      recommendations.push(
        "Consider expanding your skill set or broadening your job search criteria",
      );
    ...
    if (avgScore > 85) {
      recommendations.push(
        "Excellent profile! You're a strong candidate for most of these positions",
      );
    } else if (avgScore > 70) {
      recommendations.push(
        "Good profile with several strong matches. Consider tailoring your resume for specific roles",
      );
    } else {
      recommendations.push(
        "Consider strengthening your profile in key areas highlighted by the low-scoring matches",
      );
    }
    ...
    Object.entries(breakdown).forEach(([key, data]) => {
      if (data.score < 60) {
        switch (key) {
          case "skills_matcher":
            recommendations.push(
              "Consider acquiring additional technical skills",
            );
            break;
          ...
          case "salary_matcher":
            recommendations.push(
              "Adjust salary expectations or target higher-paying roles",
            );
            break;
        }
      }
  ...
  async loadExistingData() {
    try {
      ...
      if (database.length > 0) {
        const totalScore = database.reduce(
          (sum, record) => sum + record.averageScore,
          0,
        );
        this.performanceMetrics.averageMatchScore = Math.round(
          totalScore / database.length,
        );
      }
    } catch (error) {
  ...
  async getPerformanceMetrics() {
    return {
      ...
      jobSourcesActive: Array.from(this.jobSources.keys()).filter(
        (source) => this.jobSources.get(source).active,
      ).length,
...
if (require.main === module) {
  ...
  setTimeout(async () => {
    ...
    if (matches.topMatches.length > 0) {
      ...
      console.log(
        `💰 Salary: $${topMatch.job.salary.min.toLocaleString()} - $${topMatch.job.salary.max.toLocaleString()}`,
      );
