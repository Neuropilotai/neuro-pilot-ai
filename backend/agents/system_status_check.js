...
class SystemStatusCheck {
  ...
  async testAgentInitialization() {
    ...
    const agents = [
      ...
      {
        name: "ComplianceModerationAgent",
        file: "./compliance_moderation_agent",
      },
    ...
    for (const agent of agents) {
      try {
        ...
        if (instance.name && instance.version && instance.status) {
          ...
          console.log(
            `  ✅ ${agent.name} - ${instance.name} v${instance.version}`,
          );
        } else {
          failed.push({
            name: agent.name,
    ...
    this.results.agent_initialization = {
      working,
      failed,
  ...
  async checkDependencies() {
    ...
    for (const dep of optionalDeps) {
      try {
        ...
        console.log(
          `  ⚠️ ${dep} (optional) - not available (features will be simulated)`,
        );
      }
    }
  ...
  generateReport() {
    ...
    for (const [check, result] of Object.entries(this.results)) {
      ...
      console.log(
        `${icon} ${check.toUpperCase().replace("_", " ")}: ${status}`,
      );
    ...
    console.log(
      `❌ FAILED: ${totalChecks - passedChecks - partialChecks}/${totalChecks}`,
    );

    const overallStatus =
      passedChecks === totalChecks
        ? "READY"
        : passedChecks + partialChecks === totalChecks
