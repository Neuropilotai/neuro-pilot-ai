...
class LiveTradingAgent extends EventEmitter {
  ...
  async generateSignal() {
    try {
      ...
      console.log(
        `📊 New trading signal: ${signal.type} ${signal.symbol} (${(signal.confidence * 100).toFixed(1)}% confidence)`,
      );
    } catch (error) {
  ...
  closePosition(positionId) {
    ...
    this.performance.winRate =
      (this.performance.tradesWon / this.performance.tradesTotal) * 100;

    console.log(
      `💰 Position closed: ${position.symbol} P&L: $${pnl.toFixed(2)}`,
    );
  }
  ...
  getPerformanceMetrics() {
    ...
    const dailyPnL = closedPositions
      .filter(
        (p) =>
          new Date(p.closedAt) > new Date(Date.now() - 24 * 60 * 60 * 1000),
      )
    ...
    const weeklyPnL = closedPositions
      .filter(
        (p) =>
          new Date(p.closedAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
      )
      .reduce((sum, p) => sum + p.pnl, 0);
