...
class TradingViewProWrapper extends EventEmitter {
  ...
  handlePythonMessage(message) {
    switch (message.type) {
      ...
      case 'prediction':
        ...
        this.emit('new_prediction', { symbol: message.symbol, prediction: message.data });
        break;
  ...
  requestStatus() {
    if (this.pythonProcess && this.status === 'online') {
      try {
        this.pythonProcess.stdin.write(JSON.stringify({ command: 'get_status' }) + '\n');
      } catch (error) {
  ...
  requestPredictions() {
    if (this.pythonProcess && this.status === 'online') {
      try {
        this.pythonProcess.stdin.write(JSON.stringify({ command: 'get_predictions' }) + '\n');
      } catch (error) {
        console.error('Failed to request predictions from Python agent:', error);
      }
    }
  }
  ...
  getStatus() {
    ...
    if (this.status === 'offline' && Object.keys(this.lastStatus).length === 0) {
