...
class SalesMarketingAgent {
  ...
  async createMarketingCampaign(campaignType, targetAudience, product) {
    try {
      ...
      switch (campaignType) {
        case "social_media":
          campaign.content = await this.generateSocialMediaContent(
            product,
            targetAudience,
          );
          break;
        case "email":
          campaign.content = await this.generateEmailCampaign(
            product,
            targetAudience,
          );
          break;
        case "ad_copy":
          ...
          break;
        default:
          campaign.content = await this.generateGenericContent(
            product,
            targetAudience,
          );
      }
    ...
    try {
      ...
      this.performance_metrics.conversion_rate =
        this.performance_metrics.leads_generated > 0
          ? (campaign.performance.conversions /
              this.performance_metrics.leads_generated) *
            100
          : 0;

      await this.saveCampaignData();
      await this.log(
        `💰 Conversion tracked: Campaign ${campaignId}, Revenue: $${revenue}`,
      );
    } catch (error) {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert sales and marketing AI agent specializing in AI trading services, resume generation, and digital products. Provide specific, actionable, and results-driven marketing strategies.",
            },
            {
              role: "user",
              content: prompt,
