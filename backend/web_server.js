  ...
  setupRoutes() {
      ...
      try {
        ...
        if (!jobDescription || !candidateInfo?.name || !candidateInfo?.email) {
          return res.status(400).json({
            success: false,
            error:
              "Missing required fields: jobDescription, candidateInfo.name, candidateInfo.email",
          });
        }
        ...
        console.log(
          `🤖 Processing order for ${candidateInfo.name} (${order.package} package)`,
        );
        ...
        console.log(
          `✅ Resume generated successfully: Order ${result.order_id}`,
        );
        ...
        try {
          const paymentSession =
            await this.paymentProcessor.createPaymentSession({
          ...
          res.json({
            ...
            ai_enhancement: {
              ...
              enhanced_quality_score:
                result.ai_enhancement?.enhanced_quality_score,
    ...
    this.app.get("/api/resume/order/:orderId", async (req, res) => {
      try {
        ...
        const order = this.resumeAgent.orders.find(
        ...
        const agents = [
          {
            ...
            status: "online",
            activity:
              this.resumeAgent.orders.length > 0
                ? `Processing ${this.resumeAgent.orders.length} orders`
                : "Standby - Ready for orders",
            performance: 98.5,
  ...
  async start() {
    try {
      ...
      this.server = this.app.listen(this.port, () => {
        ...
        console.log("");
        console.log(
          "🎯 Ready to serve customers from McDonald's to Fortune 500 CEOs!",
        );
      });
