...
class EnhancedTradingAgent extends EventEmitter {
  ...
  async init() {
    try {
      ...
      console.log(`💰 Paper trading balance: $${this.paperBalance.toLocaleString()}`);
  ...
  async performLearningCycle() {
    try {
      ...
      console.log(`📚 Learning cycle completed. Progress: ${this.learningProgress.toFixed(1)}%`);
  ...
  async analyzeAndLearn(symbol, marketData) {
    try {
      ...
      // Generate prediction
      const prediction = this.generatePrediction(symbol, marketData, indicators);
  ...
  async executePaperTrade(symbol, prediction, marketData) {
    ...
    console.log(`📈 ${prediction.signal} ${shares} shares of ${symbol} at $${marketData.price} (Confidence: ${(prediction.confidence * 100).toFixed(1)}%)`);
  ...
  async checkTradeExit(tradeId) {
    ...
    try {
      ...
      // Check target price
      if ((trade.type === 'BUY' && currentPrice >= trade.targetPrice) ||
  ...
  async exitTrade(trade, exitPrice, reason) {
    ...
    console.log(`💰 Closed ${trade.type} ${trade.symbol}: ${pnl > 0 ? '+' : ''}$${pnl.toFixed(2)} (${reason})`);
  ...
  learnFromTrade(trade) {
    ...
    const confidenceAccuracy = wasSuccessful ? trade.confidence : (1 - trade.confidence);
    ...
    console.log(`🧠 Learning from trade: Model accuracy: ${(this.modelAccuracy * 100).toFixed(1)}%`);
  }
  ...
  async retrainModels() {
    ...
    if (recentData.length > 10) {
      ...
      this.modelAccuracy = Math.min(0.95, this.modelAccuracy + performanceImprovement);
      
      console.log(`✅ Model retrained. New accuracy: ${(this.modelAccuracy * 100).toFixed(1)}%`);
    }
  }
  ...
  async generateTradingSignals() {
    ...
    for (const symbol of this.symbols.slice(0, 4)) { // Limit to 4 symbols for API
      try {
        const marketData = await this.fetchMarketData(symbol);
        if (marketData) {
          const indicators = this.calculateTechnicalIndicators(marketData);
          ...
          signals.push({
            ...
            reasoning: prediction.reasoning,
            change: marketData.change ? `${marketData.change.toFixed(2)}% change` : 'Real-time analysis'
          });
        }
      } catch (error) {
  ...
  updatePerformanceMetrics() {
    if (this.performance.totalTrades > 0) {
      ...
      this.performance.avgTradeReturn = this.performance.totalPnL / this.performance.totalTrades;
    }
    
    // Log performance update
    console.log(`📊 Performance Update: Balance: $${this.paperBalance.toFixed(2)}, Total P&L: $${this.performance.totalPnL.toFixed(2)}, Win Rate: ${(this.performance.winRate * 100).toFixed(1)}%`);
  }
