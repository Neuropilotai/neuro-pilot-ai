...
class IntegratedLearningTrader extends EventEmitter {
  constructor() {
    super();
    this.tradingDataFile = path.join(
      __dirname,
      "integrated_learning_data.json",
    );
    ...
    this.allMarkets = {
      // STOCKS
      AAPL: { price: 185.5, market: "stocks", volatility: 0.02 },
      MSFT: { price: 425.3, market: "stocks", volatility: 0.015 },
      GOOGL: { price: 155.75, market: "stocks", volatility: 0.025 },
      TSLA: { price: 245.8, market: "stocks", volatility: 0.04 },
      NVDA: { price: 875.25, market: "stocks", volatility: 0.035 },

      // CRYPTO
      BTC: { price: 43250.0, market: "crypto", volatility: 0.06 },
      ETH: { price: 2650.0, market: "crypto", volatility: 0.07 },
      SOL: { price: 98.75, market: "crypto", volatility: 0.12 },
      ADA: { price: 0.52, market: "crypto", volatility: 0.15 },
      DOGE: { price: 0.085, market: "crypto", volatility: 0.2 },
      ...
      // COMMODITIES
      GOLD: { price: 2045.5, market: "commodities", volatility: 0.015 },
  ...
  async loadExistingLearning() {
    try {
      ...
      console.log(
        "🧠 Integrated existing learning with new trading capabilities",
      );
    } catch (error) {
  ...
  async saveState() {
    ...
    await fs.writeFile(
      ...
      JSON.stringify(this.learningAgent, null, 2),
    );
  }
  ...
  async learnFromTrade(trade) {
    ...
    if (trade.pnl > 0) {
      ...
      console.log(
        `🧠 AI LEARNED: ${patternKey} pattern (+$${trade.pnl.toFixed(2)})`,
      );
    } else if (trade.pnl < 0) {
      // Learn from losses
      const failPattern = {
      ...
      console.log(
        `🧠 AI LEARNING FROM LOSS: Avoid ${failPattern.market} ${failPattern.strategy} (-$${Math.abs(trade.pnl).toFixed(2)})`,
      );
    }
  ...
  updateExpertiseLevel() {
    ...
    if (expertise !== this.learningAgent.learningMetrics.expertise_level) {
      console.log(
        `🎓 AI EXPERTISE UPGRADE: ${this.learningAgent.learningMetrics.expertise_level} → ${expertise}`,
      );
  ...
  async generateLearningBasedSignals() {
    ...
    for (const [symbol, data] of Object.entries(this.allMarkets)) {
      ...
      let signal = await this.generateIntelligentSignal(
        symbol,
        data,
        currentHour,
      );
  ...
  async generateIntelligentSignal(symbol, data, hour) {
    ...
    for (const [patternKey, patterns] of this.discoveredPatterns) {
      if (patternKey.includes(data.market)) {
        ...
        if (successfulPatterns.length > 0) {
          const avgProfit =
            successfulPatterns.reduce((sum, p) => sum + p.profitMargin, 0) /
            successfulPatterns.length;

          // If we've learned this market/time combination is profitable
          const timeMatch = successfulPatterns.some(
            (p) => Math.abs(p.timeOfDay - hour) <= 1,
          );
          if (timeMatch && avgProfit > 0.02) {
            signal += 0.3; // Strong positive signal based on learning
            console.log(
              `🧠 AI APPLYING LEARNED PATTERN: ${patternKey} (+${avgProfit.toFixed(3)})`,
            );
          }
        }
      }
    ...
    if (this.learningAgent.learningMetrics.expertise_level === "expert") {
      ...
      this.learningAgent.learningMetrics.expertise_level === "advanced"
    ) {
      signal *= 1.15;
    }
  ...
  selectBestStrategy(market) {
    ...
    const marketStrategies = Array.from(this.tradingStrategies.keys()).filter(
      (key) => key.includes(market),
    );
  ...
  async executeIntelligentTrade(signal) {
    ...
    const position = this.wallet.positions.get(symbol) || {
      quantity: 0,
      avgPrice: 0,
    };
    ...
    const pnlStr =
      trade.pnl !== 0
        ...
        : "";
    console.log(
      `🤖 ${this.learningAgent.learningMetrics.expertise_level.toUpperCase()} ${action} ${quantity} ${symbol} @ $${price.toFixed(4)} [${strategy}]${pnlStr}`,
    );
  ...
  async runLearningTradingCycle() {
    ...
    console.log(
      `🎓 Current expertise: ${this.learningAgent.learningMetrics.expertise_level}`,
    );
    console.log(
      `📊 Model confidence: ${(this.learningAgent.learningMetrics.model_confidence * 100).toFixed(1)}%`,
    );
    ...
    if (signals.length === 0) {
      console.log(
        "🤔 AI needs more learning data - no confident signals generated",
      );
      return;
    }
    ...
    const maxTrades =
      this.learningAgent.learningMetrics.expertise_level === "expert"
        ? 4
        : this.learningAgent.learningMetrics.expertise_level === "advanced"
          ? 3
          : 2;
  ...
  displayLearningStats() {
    ...
    const accuracy =
      metrics.total_predictions > 0
        ? (
            (metrics.successful_trades / metrics.total_predictions) *
            100
          ).toFixed(1)
        : "0.0";
    ...
    console.log("\\n🧠 AI LEARNING STATS:");
    console.log(
      `💰 Balance: $${this.wallet.currentBalance.toFixed(2)} | P&L: ${this.wallet.totalPnL >= 0 ? "+" : ""}$${this.wallet.totalPnL.toFixed(2)}`,
    );
    console.log(
      `🎓 Expertise: ${metrics.expertise_level} | Confidence: ${(metrics.model_confidence * 100).toFixed(1)}%`,
    );
    console.log(
      `📈 Trading Accuracy: ${accuracy}% | Trades: ${metrics.total_predictions}`,
    );
    console.log(
      `🔍 Patterns Learned: ${this.discoveredPatterns.size} | Iterations: ${metrics.learning_iterations}`,
    );
    ...
    if (this.wallet.positions.size > 0) {
      ...
      for (const [symbol, pos] of this.wallet.positions) {
        ...
        console.log(
          `   ${symbol}: ${pos.quantity} @ $${pos.avgPrice.toFixed(4)} | Unrealized: ${unrealizedPnL >= 0 ? "+" : ""}$${unrealizedPnL.toFixed(2)}`,
        );
      }
    }
  }
  ...
  async getCurrentState() {
    return {
      wallet: {
        ...this.wallet,
        positions: Array.from(this.wallet.positions.entries()).map(
          ([symbol, pos]) => ({
            symbol,
            ...pos,
            currentPrice: this.allMarkets[symbol]?.price || 0,
            unrealizedPnL: this.allMarkets[symbol]
              ? (this.allMarkets[symbol].price - pos.avgPrice) * pos.quantity
              : 0,
          }),
        ),
      },
