...
async function comprehensiveResumeTest() {
  ...
  const englishOrder = {
    id: Date.now(),
    jobDescription: 'Senior Full-Stack Developer with expertise in React, Node.js, and cloud technologies',
    candidateInfo: {
  ...
  try {
    ...
    if (englishResult.error) {
      ...
      console.log('  Design Quality:', englishResult.canva_design?.design_quality);
      console.log('  Export Formats:', englishResult.canva_design?.export_formats?.join(', '));
    }
  } catch (error) {
  ...
  const frenchOrder = {
    id: Date.now() + 1,
    jobDescription: 'Directeur Marketing Digital avec expertise en stratégie numérique et analytics',
    candidateInfo: {
      name: 'Marie-Claire Dubois',
      experience: '12 ans marketing digital',
      skills: 'Stratégie Digitale, SEO/SEM, Analytics, Leadership, Automation Marketing',
  ...
  try {
    ...
    if (frenchResult.error) {
      ...
      console.log('  Design Quality:', frenchResult.canva_design?.design_quality);
    }
  } catch (error) {
  ...
  try {
    ...
    console.log('  Resume Packages Available:', Object.keys(pricing.resume_packages || {}).length);
    console.log('  Basic Package Price:', pricing.resume_packages?.basic?.price || 'N/A');
    console.log('  Professional Package Price:', pricing.resume_packages?.professional?.price || 'N/A');
    console.log('  Executive Package Price:', pricing.resume_packages?.executive?.price || 'N/A');
  ...
  console.log('  Canva Pro Status:', canvaStatus.enabled ? '✅ Enabled' : '⚠️ Simulated Mode');
  console.log('  API Connected:', canvaStatus.api_connected ? '✅ Connected' : '⚠️ Mock Mode');
  console.log('  Templates Available:', canvaStatus.templates_available);
  console.log('  Bilingual Support:', canvaStatus.bilingual_support ? '✅ Yes' : '❌ No');
  console.log('  Languages Supported:', canvaStatus.languages.join(', '));
  console.log('  Package Templates:', canvaStatus.template_categories.package_templates.join(', '));
  console.log('  Custom Templates:', canvaStatus.template_categories.custom_templates.join(', '));
  console.log('  Total Designs:', canvaStatus.template_categories.total_designs);
  ...
  Object.keys(templates.package_templates).forEach(pkg => {
    console.log(`    ${pkg}:`, Object.keys(templates.package_templates[pkg]).join(', '));
  });
  console.log('  Custom Templates:');
  Object.keys(templates.custom_templates).forEach(template => {
    console.log(`    ${template}:`, Object.keys(templates.custom_templates[template]).join(', '));
  });
  ...
  console.log('Canva:', process.env.CANVA_API_KEY && process.env.CANVA_API_KEY !== 'your_canva_api_key_here' ? '🔑 Configured' : '⚠️ Placeholder');
