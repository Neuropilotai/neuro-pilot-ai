...
class SuperAgent {
  constructor() {
    ...
    this.services = [
      ...
      {
        name: "Fiverr Pro",
        process: "fiverr_pro_enhanced.js",
        port: 8082,
        critical: false,
      },
      {
  ...
  async init() {
    ...
    console.log(
      `⚙️ Auto-fix: ${this.config.autoFixEnabled ? "ENABLED" : "DISABLED"}`,
    );
    console.log(
      `📚 Learning: ${this.config.learningEnabled ? "ENABLED" : "DISABLED"}`,
    );
    console.log(
      `🗂️ File Organization: ${this.config.fileOrganizationEnabled ? "ENABLED" : "DISABLED"}`,
    );
  }
  ...
  async checkServiceStatus(service) {
    try {
      ...
      const { stdout } = await execAsync(
        `ps aux | grep "${service.process}" | grep -v grep`,
      );
  ...
  async handleServiceUp(service, status) {
    await this.log("SERVICE_UP", `✅ ${service.name} is back online`, {
      service: service.name,
      status,
    });
  ...
  async attemptServiceRestart(service, issue) {
    ...
    try {
      ...
      if (newStatus.running) {
        ...
        await this.log(
          "AUTO_FIX_SUCCESS",
          `✅ Successfully restarted ${service.name}`,
          fix,
        );
      ...
      await this.log(
        "AUTO_FIX_ERROR",
        `❌ Error restarting ${service.name}: ${error.message}`,
  ...
  getRestartCommand(service) {
    const commands = {
      "simple_email_agent.js":
        "nohup node simple_email_agent.js > email_agent.log 2>&1 &",
      "server.js": "PORT=8080 nohup node server.js > server_8080.log 2>&1 &",
      "admin_server.js":
        "PORT=8081 nohup node admin_server.js > admin_8081.log 2>&1 &",
      "fiverr_pro_enhanced.js":
        "PORT=8082 nohup node fiverr_pro_enhanced.js > fiverr_pro.log 2>&1 &",
      "agent_dashboard.js":
        "PORT=3005 nohup node agent_dashboard.js > dashboard.log 2>&1 &",
      "enhanced_dashboard.js":
        "PORT=3006 nohup node enhanced_dashboard.js > enhanced_dashboard.log 2>&1 &",
      "ngrok_manager.js":
        "nohup node ngrok_manager.js > ngrok_manager.log 2>&1 &",
    };

    return (
      commands[service.process] ||
      `nohup node ${service.process} > ${service.process}.log 2>&1 &`
    );
  }
  ...
  async handleHighMemoryUsage(resources) {
    ...
    await this.log(
      "HIGH_MEMORY",
      `⚠️ High memory usage: ${resources.memoryUsage}%`,
      issue,
    );
    ...
    try {
      ...
      for (const service of nonCriticalServices) {
        ...
        if (status.running) {
          ...
          await this.log(
            "SERVICE_RESTART",
            `🔄 Restarted ${service.name} for memory optimization`,
          );
        }
      }
    } catch (error) {
      await this.log(
        "OPTIMIZATION_ERROR",
        `❌ Memory optimization failed: ${error.message}`,
      );
    }
  }
  ...
  async checkNgrokHealth() {
    try {
      ...
      if (!tunnels.tunnels || tunnels.tunnels.length === 0) {
        ...
        const httpsUrl = tunnels.tunnels.find((t) =>
  ...
  async handleNgrokDown() {
    ...
    if (this.config.autoFixEnabled) {
      try {
        ...
        await this.log(
          "NGROK_RESTART_FAILED",
          `❌ Failed to restart ngrok: ${error.message}`,
        );
      }
    }
  }
  ...
  async updateNgrokUrl(newUrl) {
    try {
      ...
      const currentUrl = await fs
        .readFile(currentUrlFile, "utf8")
        .catch(() => "");
