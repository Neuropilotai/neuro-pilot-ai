...
async function testPayments() {
  ...
  try {
    ...
    const resumeResponse = await fetch(`${baseUrl}/api/payments/resume-checkout`, {
      method: 'POST',
    ...
    console.log('✅ Resume Checkout:', resumeResult.status === 'success' ? 'SUCCESS' : 'FAILED');
    ...
    const subResponse = await fetch(`${baseUrl}/api/payments/trading-subscription`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.log('✅ Trading Subscription:', subResult.status === 'success' ? 'SUCCESS' : 'FAILED');
