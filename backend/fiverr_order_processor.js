...
class FiverrOrderProcessor {
  constructor() {
    this.apiUrl = process.env.PRODUCTION_URL
      ? `${process.env.PRODUCTION_URL}/api/resume/generate`
      : process.env.RAILWAY_URL
        ? `${process.env.RAILWAY_URL}/api/resume/generate`
  ...
  async start() {
    ...
    console.log("========================");
    console.log(
      "This tool helps you process Fiverr orders through your AI agent\n",
    );
  ...
  async processNewOrder() {
    ...
    const orderInfo = {
      ...
      jobDescription: await this.askMultilineQuestion(
        "Job Description (paste here, then press Enter twice):\n",
      ),
...
Best of luck with your application! 🚀`;
  ...
  async createOrderTemplate() {
    ...
    const template = {
      instructions:
        "Fill in this template with customer information from Fiverr",
    ...
    console.log(
      "Edit this file with customer info, then use option 3 to batch process",
    );
  }
    ...
    for (const file of files) {
      ...
      try {
        const orderData = JSON.parse(
          fs.readFileSync(path.join(this.ordersDir, file), "utf8"),
        );
