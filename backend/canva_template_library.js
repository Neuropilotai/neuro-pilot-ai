...
class CanvaTemplateLibrary {
  ...
  getTemplateForJob(
    ...
    if (packageTemplates && packageTemplates[industry]) {
      return {
        ...
        language: language,
        color_scheme:
          this.colorSchemes[packageTemplates[industry].color_scheme],
  ...
  getTemplateRecommendations(jobAnalysis) {
    ...
    return {
      primary_recommendation: this.getTemplateForJob(
        jobAnalysis,
        recommendedPackage,
      ),
      alternative_options: this.getAlternativeTemplates(
        category,
        industry,
        recommendedPackage,
      ),
  ...
  getAlternativeTemplates(category, industry, packageType) {
    ...
    if (categoryTemplates) {
      Object.keys(categoryTemplates).forEach((pkg) => {
        if (pkg !== packageType) {
          const template =
            categoryTemplates[pkg]?.[industry] ||
            categoryTemplates[pkg]?.general;
          if (template) {
            alternatives.push({
              package: pkg,
              template: template,
  ...
  getDesignReasoning(jobAnalysis) {
    ...
    return (
      ...
      `Professional design tailored for ${industry} industry standards`
    );
  }
  ...
  getLayoutSpecs(templateInfo, jobAnalysis) {
    return {
      ...
      header_style: templateInfo.features.includes("modern_typography")
        ? "modern"
        : "classic",
