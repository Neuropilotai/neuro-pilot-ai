...
class CanvaCLIIntegration {
  ...
  async generateResumeDesign(
    ...
    try {
      console.log(
        `🎨 Generating Canva design for ${packageType} resume in ${language}...`,
      );

      if (!this.cliAvailable) {
        return this.simulateCanvaDesign(
          resumeData,
          packageType,
          language,
          customTemplate,
      ...
      const designResult = await this.createDesignWithCLI(
        resumeData,
        templateId,
      );
  ...
  async setupAuthentication() {
    if (!this.cliAvailable) {
      throw new Error(
        "Canva CLI not available. Please install with: npm install -g @canva/cli@latest",
      );
    }
    ...
    console.log(
      "3. Create your first app: canva apps create neuro-pilot-resume",
    );
