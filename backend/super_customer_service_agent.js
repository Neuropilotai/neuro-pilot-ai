...
`
    ...
    startEmailMonitoring() {
        setInterval(async () => {
            ...
            try {
                ...
                console.log(`[${new Date().toLocaleTimeString()}] 📧 Monitoring emails and customer service...`);
            } catch (error) {
    ...
    async processEmailFile(filePath) {
        try {
            ...
            const processedPath = path.join(this.emailFolder, 'processed', path.basename(filePath));
    ...
    async classifyAndProcessEmail(emailData) {
        ...
        console.log(`📧 Classified email as: ${classification.type} (confidence: ${classification.confidence}%)`);
    ...
    async classifyEmail(emailData) {
        ...
        const content = (emailData.content || emailData.message || '').toLowerCase();
        const fullText = `${subject} ${content}`;
        
        // Classification logic
        if (this.containsKeywords(fullText, ['urgent', 'asap', 'emergency', 'rush', 'immediately', 'today'])) {
            return { type: 'urgent_request', confidence: 95, keywords: ['urgent'] };
        }
        
        if (this.containsKeywords(fullText, ['order', 'resume', 'buy', 'purchase', 'need resume', 'create resume'])) {
            return { type: 'order_request', confidence: 90, keywords: ['order', 'resume'] };
        }
        
        if (this.containsKeywords(fullText, ['price', 'cost', 'how much', 'pricing', 'packages', 'plans'])) {
            return { type: 'pricing_inquiry', confidence: 85, keywords: ['pricing'] };
        }
        
        if (this.containsKeywords(fullText, ['help', 'support', 'problem', 'issue', 'question', 'how to'])) {
            return { type: 'support_inquiry', confidence: 80, keywords: ['support'] };
        }
        
        if (this.containsKeywords(fullText, ['hello', 'hi', 'interested', 'learn more', 'tell me'])) {
    ...
    async extractOrderInfo(emailData) {
        ...
        const orderInfo = {
            fullName: this.extractField(content, ['name:', 'my name is', 'i am', 'i\'m']) || emailData.from || 'Valued Customer',
            email: emailData.email || emailData.from || 'customer@example.com',
            targetRole: this.extractField(content, ['role:', 'position:', 'job:', 'title:']) || 'Professional',
            industry: this.extractField(content, ['industry:', 'field:', 'sector:']) || 'General',
            experience: this.extractField(content, ['experience:', 'years:', 'worked for']) || '5+ years',
            keywords: this.extractField(content, ['skills:', 'keywords:', 'expertise:']) || '',
            package: this.detectPackage(content),
            isComplete: false
        };
        
        // Check if order is complete
        orderInfo.isComplete = orderInfo.fullName !== 'Valued Customer' && 
    ...
    detectPackage(content) {
        ...
        if (lowerContent.includes('executive') || lowerContent.includes('premium') || lowerContent.includes('$99')) {
            return 'executive';
        }
    ...
    async sendAutoResponse(emailData, templateType, variables = {}) {
        ...
        // Send response
        try {
            const customerEmail = emailData.email || emailData.from || 'customer@example.com';
            ...
            console.log(`📧 Auto-response sent to ${customerEmail} (Template: ${templateType})`);
    ...
    async getCustomerServiceStats() {
        ...
        try {
            ...
            const processedFiles = await fs.readdir(path.join(this.emailFolder, 'processed'));
...
if (require.main === module) {
    ...
    setTimeout(async () => {
        ...
        const sampleInquiry = {
            ...
            message: 'Hi, I am John Smith and I need a professional resume. Can you tell me about your pricing and packages? I am targeting a software engineer position in the tech industry.',
            timestamp: new Date().toISOString()
        };
