...
router.get('/orders/:orderId/download', authenticateToken, async (req, res) => {
    try {
        ...
        if (format === 'html') {
            ...
            res.setHeader('Content-Disposition', `attachment; filename="resume_${order.orderId}.html"`);
            res.send(htmlContent);
        } else if (format === 'txt') {
            const textContent = generateTextResume(resumeData);
            res.setHeader('Content-Type', 'text/plain');
            res.setHeader('Content-Disposition', `attachment; filename="resume_${order.orderId}.txt"`);
            res.send(textContent);
        } else if (format === 'json') {
            res.setHeader('Content-Type', 'application/json');
            res.setHeader('Content-Disposition', `attachment; filename="resume_${order.orderId}.json"`);
            res.json(resumeData);
        } else {
            res.status(400).json({ error: 'Unsupported format. Use html, txt, or json' });
        }
    } catch (error) {
...
function generateHTMLResume(resumeData, packageType) {
    ...
    const summary = resumeData.summary || 'Professional summary will be displayed here.';
    ...
    const bgColor = isPremium ? 'linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)' : '#ffffff';
...
`).join('\n');
    ...
    const certificationsText = certifications.length > 0 ? `
    ...
    try {
        ...
        res.json({
            stats: {
                ...
                averageOrderValue: totalOrders.count > 0 ? (totalSpent.total || 0) / totalOrders.count : 0
            },
            recentOrders
        });
    } catch (error) {
...
router.put('/preferences', authenticateToken, async (req, res) => {
    try {
        ...
        `, [req.user.id, language || 'en', resumeStyle || 'professional', notifications !== false]);
