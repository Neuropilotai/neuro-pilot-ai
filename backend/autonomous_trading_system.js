...
class AutonomousTradingSystem extends EventEmitter {
  constructor() {
    ...
    this.marketData = {
      AAPL: { price: 185.5, volatility: 0.02, trend: 1 },
      MSFT: { price: 425.3, volatility: 0.015, trend: 1 },
      GOOGL: { price: 155.75, volatility: 0.025, trend: 0 },
      TSLA: { price: 245.8, volatility: 0.04, trend: -1 },
      NVDA: { price: 875.25, volatility: 0.035, trend: 1 },
      META: { price: 485.9, volatility: 0.03, trend: 0 },
      AMZN: { price: 175.2, volatility: 0.02, trend: 1 },
    };
  ...
  analyzeTradingPatterns() {
    ...
    this.wallet.learningMetrics.winRate =
      (wins.length / recentTrades.length) * 100;
    this.wallet.learningMetrics.avgWinAmount =
      wins.length > 0
        ? wins.reduce((sum, t) => sum + t.pnl, 0) / wins.length
        : 0;
    this.wallet.learningMetrics.avgLossAmount =
      losses.length > 0
    ...
    console.log(
      `   Win Rate: ${this.wallet.learningMetrics.winRate.toFixed(1)}%`,
    );
  ...
  async executeTrade(signal) {
    ...
    if (action === "BUY") {
      ...
      if (trade.value > this.wallet.currentBalance * 0.8) {
        console.log(
          `⚠️ Insufficient funds for ${symbol} trade ($${trade.value.toFixed(2)} needed, $${this.wallet.currentBalance.toFixed(2)} available)`,
        );
        return null;
      }
    ...
    const pnlStr =
      trade.pnl !== 0
        ...
        : "";
    console.log(
      `🤖 ${action} ${quantity} ${symbol} @ $${price.toFixed(2)} [${strategy}]${pnlStr}`,
    );
  ...
  displayStats() {
    ...
    console.log(
      `💰 Balance: $${this.wallet.currentBalance.toFixed(2)} | Total P&L: ${this.wallet.totalPnL >= 0 ? "+" : ""}$${this.wallet.totalPnL.toFixed(2)}`,
    );
    console.log(
      `📈 Win Rate: ${winRate.toFixed(1)}% | Total Trades: ${totalTrades}`,
    );
    console.log(
      `🎯 Best Strategy: ${this.wallet.learningMetrics.bestStrategy || "Learning..."}`,
    );
    ...
    if (this.wallet.positions.size > 0) {
      ...
      for (const [symbol, pos] of this.wallet.positions) {
        ...
        console.log(
          `   ${symbol}: ${pos.quantity} shares @ $${pos.avgPrice.toFixed(2)} | Unrealized: ${unrealizedPnL >= 0 ? "+" : ""}$${unrealizedPnL.toFixed(2)}`,
        );
      }
    }
  }
      ...
      wallet: {
        ...
        positions: Array.from(this.wallet.positions.entries()).map(
          ([symbol, pos]) => ({
            ...
            unrealizedPnL:
              (this.marketData[symbol].price - pos.avgPrice) * pos.quantity,
          }),
        ),
      },
      strategies: this.strategies,
