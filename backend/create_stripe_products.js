...
async function createStripeProducts() {
  try {
    ...
    const productIds = {
      subscriptions: {
        basic: { product: neuroBasic.id, price: neuroBasicPrice.id, amount: 29 },
        pro: { product: neuroPro.id, price: neuroProPrice.id, amount: 99 },
        enterprise: { product: neuroEnterprise.id, price: neuroEnterprisePrice.id, amount: 299 }
      },
      one_time: {
        ai_models: { product: premiumAIModels.id, price: premiumAIModelsPrice.id, amount: 199 },
        historical_data: { product: historicalData.id, price: historicalDataPrice.id, amount: 49 }
      },
      resume: {
        basic: { product: resumeBasic.id, price: resumeBasicPrice.id, amount: 29 },
        professional: { product: resumeProfessional.id, price: resumeProfessionalPrice.id, amount: 59 },
        executive: { product: resumeExecutive.id, price: resumeExecutivePrice.id, amount: 99 }
      }
    };

    // Save to file for reference
    require('fs').writeFileSync('stripe_products.json', JSON.stringify(productIds, null, 2));
...
if (require.main === module) {
  ...
  if (!process.env.STRIPE_SECRET_KEY || process.env.STRIPE_SECRET_KEY.includes('PASTE_YOUR')) {
    console.error('❌ Please add your actual Stripe Secret Key to the .env file first!');
    console.log('📝 Edit .env and replace STRIPE_SECRET_KEY with your real key from Stripe Dashboard');
    process.exit(1);
  }
