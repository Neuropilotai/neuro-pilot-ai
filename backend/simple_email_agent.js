...
class SimpleEmailAgent {
  ...
  async monitorOrders() {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        console.log(
          `[${new Date().toLocaleTimeString()}] ✅ Email agent monitoring...`,
        );
      } catch (error) {
  ...
  async processEmailOrder(orderData) {
    try {
      ...
      if (orderData.cvFile) {
        console.log(
          `📎 Using uploaded CV: ${orderData.cvFile.originalName || orderData.cvFile.filename}`,
        );
  ...
  showUsageExample() {
    ...
    console.log(
      "1️⃣ When you receive a customer email with order details, create a JSON file:",
    );
