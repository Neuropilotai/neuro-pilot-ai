...
router.post('/email-attachment', async (req, res) => {
    try {
        ...
        if (attachments && attachments.length > 0) {
            ...
            const result = await emailSystem.processEmailWithAttachment(from, attachment);
...
router.post('/deliver-resume', async (req, res) => {
    try {
        ...
        // Load order data
        const orderPath = path.join(__dirname, '../orders', `order_${orderId}.json`);
        const orderData = JSON.parse(await require('fs').promises.readFile(orderPath, 'utf8'));
        ...
        if (result.success) {
            ...
            await require('fs').promises.writeFile(orderPath, JSON.stringify(orderData, null, 2));
