 ...
 * 6. Click Run → setupInventorySystem OR importFromOrder05042025
...
function importGFSData(spreadsheet) {
  ...
  const possibleSheetNames = ['Order 05042025', 'GFS Order July 4 2025', 'Sheet1', 'GFS_Order_July4_2025'];
  ...
  if (!sourceSheet) {
    sourceSheet = spreadsheet.getSheets()[0];
  }
...
function createCustomMenu() {
  ...
  ui.createMenu('🏔️ Camp Inventory')
    ...
    .addSeparator()
    .addItem('🔧 Import Order 05042025', 'importFromOrder05042025')
...
// ===== IMPORT FROM ORDER 05042025 =====
function importFromOrder05042025() {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const sourceSheet = spreadsheet.getSheetByName('Order 05042025');
  const targetSheet = spreadsheet.getSheetByName('Inventory_Master');
  
  if (!sourceSheet) {
    SpreadsheetApp.getUi().alert('Cannot find "Order 05042025" sheet!');
    return;
  }
  ...
  if (targetLastRow > 1) {
    targetSheet.getRange(2, 1, targetLastRow - 1, 17).clearContent();
  }
  ...
  // Get all source data (assuming 7 columns: Product#, Name, Brand, Format, Price, Qty, Total)
  const sourceData = sourceSheet.getRange(2, 1, sourceLastRow - 1, 7).getValues();
  ...
  sourceData.forEach(row => {
    if (row[0]) { // If product # exists
      let location = 'Dry Storage 1';
      const productName = (row[1] || '').toString().toLowerCase();
      
      // Smart location assignment based on product name
      if (productName.includes('frais') || productName.includes('fresh')) {
        location = 'Walk-in Cooler';
      } else if (productName.includes('congel') || productName.includes('frozen') || productName.includes('iqf')) {
        location = 'Walk-in Freezer';
      } else if (productName.includes('boeuf') || productName.includes('poulet') || productName.includes('porc')) {
        location = productName.includes('congel') ? 'Walk-in Freezer' : 'Walk-in Cooler';
      } else if (productName.includes('papier') || productName.includes('serviette') || productName.includes('paper')) {
        location = 'Paper Goods';
      } else if (productName.includes('nettoy') || productName.includes('clean') || productName.includes('savon')) {
        location = 'Chemical Storage';
      } else if (productName.includes('lait') || productName.includes('fromage') || productName.includes('yaourt')) {
        location = 'Walk-in Cooler';
      } else if (productName.includes('pain') || productName.includes('bread') || productName.includes('boulang')) {
        location = 'Dry Storage 1';
      }
      
      // Calculate min/max stock levels
      const orderQty = parseFloat(row[5]) || 1;
      const minStock = Math.ceil(orderQty * 0.5);
      const maxStock = Math.ceil(orderQty * 2);
      ...
      if (productName.includes('boeuf') || productName.includes('poulet') || 
          productName.includes('porc') || productName.includes('beef') || 
          productName.includes('chicken') || productName.includes('pork') ||
          productName.includes('viande') || productName.includes('meat')) {
        notes = 'Emergency protein';
      }
      ...
      processedData.push([
        ...
        '', // Last Count Date
        '', // Last Count By
        1, // Usage Rate (formula will override)
        999, // Days Until Stockout (formula will override)
        'OUT OF STOCK', // Alert Status (formula will override)
        notes // Notes
      ]);
    }
  });
  
  // Write to Inventory_Master
  if (processedData.length > 0) {
    targetSheet.getRange(2, 1, processedData.length, 17).setValues(processedData);
    
    // Apply formulas to each row
    for (let i = 2; i <= processedData.length + 1; i++) {
      // Reorder Qty formula
      targetSheet.getRange(i, 10).setFormula('=IF(G' + i + '<H' + i + ',I' + i + '-G' + i + '-K' + i + ',0)');
      // Usage Rate formula (simplified)
      targetSheet.getRange(i, 14).setFormula('=1');
      // Days Until Stockout formula
      targetSheet.getRange(i, 15).setFormula('=IF(N' + i + '>0,G' + i + '/N' + i + ',999)');
      // Alert Status formula
      targetSheet.getRange(i, 16).setFormula('=IF(G' + i + '=0,"OUT OF STOCK",IF(G' + i + '<H' + i + '*0.5,"CRITICAL",IF(G' + i + '<H' + i + ',"LOW","OK")))');
    }
    
    // Log the import
    logAIAction('DATA_IMPORT', '', '', 'INFO', 
      `Imported ${processedData.length} products from Order 05042025`, '', '', 'Success');
    
    SpreadsheetApp.getUi().alert('Success!', `Imported ${processedData.length} products from Order 05042025\n\nNext steps:\n1. Review locations in Inventory_Master\n2. Adjust min/max levels if needed\n3. Generate AI suggestions`, SpreadsheetApp.getUi().ButtonSet.OK);
  } else {
    SpreadsheetApp.getUi().alert('No valid data found to import');
  }
}
...
<div style="font-family: Arial, sans-serif; padding: 20px; line-height: 1.6;">
  ...
  <ol>
    <li><strong>Import Data:</strong> Use "Import Order 05042025" from the menu</li>
    <li><strong>Review Locations:</strong> Check assigned storage locations in Inventory_Master</li>
    <li><strong>Set Stock Levels:</strong> Adjust min/max levels based on your experience</li>
    <li><strong>Generate Suggestions:</strong> Use AI to analyze and suggest orders</li>
  </ol>
  
  <h3>🤖 AI Features</h3>
  <ul>
    <li><strong>Smart Suggestions:</strong> Analyzes stock levels and rotation needs</li>
    <li><strong>Emergency Alerts:</strong> Monitors protein buffer (30+ days required)</li>
    <li><strong>Location Assignment:</strong> Auto-assigns storage based on product type</li>
    <li><strong>Usage Tracking:</strong> Learns consumption patterns over time</li>
  </ul>
  
  <h3>📋 Daily Operations</h3>
  <ul>
    <li><strong>Count Sheets:</strong> Print location-specific sheets for staff</li>
    ...
    <li><strong>Emergency Check:</strong> Verify protein buffer weekly</li>
  </ul>
  ...
  <h3>📅 Remote Camp Schedule</h3>
  <ul>
    <li>2-week rotations (Team A / Team B)</li>
    <li>4-week menu cycle</li>
    <li>Wednesday to Tuesday weeks</li>
    <li>Orders: Sunday/Monday placement, Friday noon cutoff</li>
    <li>Delivery delays common (train/freight to Schefferville)</li>
  </ul>
  
  <h3>📍 Storage Locations</h3>
  <ul>
    <li>Dry Storage 1 & 2</li>
    <li>Walk-in Cooler & Freezer</li>
    <li>Reach-in Fridge 1 & 2</li>
    <li>Chest Freezer 1 & 2</li>
    <li>Chemical Storage & Paper Goods</li>
    <li>Kitchen Active (daily use items)</li>
  </ul>
  
  <h3>💡 Best Practices</h3>
  <ul>
    <li>Update physical counts before generating orders</li>
    <li>Check emergency protein weekly (30+ days required)</li>
    <li>Account for delivery delays in remote locations</li>
    <li>Use air freight only for critical emergencies</li>
    <li>Plan for 200-400 person capacity fluctuations</li>
  </ul>
  
  <h3>🔧 Quick Actions</h3>
  <ul>
    <li><strong>Import Data:</strong> 🔧 Import Order 05042025</li>
    <li><strong>Check Stock:</strong> 🚨 Check Emergency Stock</li>
    <li><strong>Make Order:</strong> 🤖 Generate AI Suggestions → 📦 Create New Order</li>
    <li><strong>Count Inventory:</strong> 🖨️ Prepare Sheets for Printing</li>
  </ul>
  
  <h3>❓ Support</h3>
  <p>For technical issues or system questions, contact your IT department.</p>
</div>
  `;
  ...
  SpreadsheetApp.getUi().showModalDialog(htmlOutput, 'Mining Camp Inventory System Help');
}
...
function debugImport() {
  ...
  if (!sourceSheet) {
    SpreadsheetApp.getUi().alert('Sheet "Order 05042025" not found!');
    return;
  }
  ...
  const firstDataRow = sourceSheet.getRange(2, 1, 1, lastCol).getValues()[0];
  
  let message = `Found sheet with ${lastRow} rows and ${lastCol} columns\n\n`;
  message += `Headers: ${headers.join(', ')}\n\n`;
  message += `First product: ${firstDataRow[0]} - ${firstDataRow[1]}\n\n`;
